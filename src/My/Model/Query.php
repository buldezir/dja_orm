...
//        dump($sql); return;
    ...
    {
        $parts = array();
        foreach ($this->columns as $colName => $alias) {
            if (is_int($colName)) {
                $colName = $alias;
                $alias = 't';
            }
            if (!is_string($colName) && $colName instanceof Expr) {
                ...
                if ($colName != self::SQL_STAR) {
                    $parts[] = $this->db->quoteId($alias.'.'.$colName);
                } else {
                    $parts[] = $this->db->quoteId($alias).'.'.self::SQL_STAR;
                }
            }
        }
    ...
    protected function exec()
    {
        if ($this->pdostatement === null) {
            // |\PDO::FETCH_PROPS_LATE
            $this->pdostatement = $this->db->query($this->buildQuery(), \PDO::FETCH_CLASS, $this->metadata->getModelClass(), array('isNewRecord' => false));
    ...
    {
        return $this->db->query($this->buildQuery())->fetchAll();
    }
    ...
    public function current()
    {
        if ($this->forceNoCache) {
            ...
            if (!isset($this->rowCache[$this->pointer])) {
                $this->rowCache[$this->pointer] = $this->exec()->fetch();
            }
